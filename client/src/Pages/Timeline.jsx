...
function StepCard({ step, index, onSubmit, submitting }) {
  ...
  const criteria = step.criteria
    ? (typeof step.criteria === "string" ? JSON.parse(step.criteria) : step.criteria)
    : [];
        ...
        <div className="p-5 pb-0">
          <div className="flex items-start justify-between gap-3 mb-2">
            <h3 className={`font-semibold text-sm leading-tight
              ${isDone ? "text-green-700" : isCurrent ? "text-gray-900" : "text-gray-400"}`}>
              {step.step_title}
            </h3>
          ...
          <div className="px-5 pb-5">
                ...
                <ul className="space-y-2">
                  {tasks.map((t, i) => (
                    <li key={i} className="flex items-start gap-2 text-xs text-gray-700">
                      <span className="text-[#F26419] font-bold mt-0.5 flex-shrink-0">›</span>
                      {t}
                    </li>
                  ))}
                </ul>
            ...
            {criteria.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mb-4">
                {criteria.map((c, i) => (
                  <span key={i} className="text-[11px] bg-[#FEF0E8] text-[#F26419] border border-[#F26419]/20 px-2.5 py-1 rounded-full font-medium">
                    {c}
                  </span>
                ))}
              </div>
                ...
                <div className="bg-green-50 border border-green-100 rounded-xl p-4">
                  ...
                  {step.submitted_at && (
                    <p className="text-[10px] text-gray-400 mt-2">
                      Enviado el {new Date(step.submitted_at).toLocaleDateString("es-CO", {
                ...
                {step.feedback_text ? (
                  <div className="bg-[#1C1712] rounded-xl p-4">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <p className="text-[11px] font-bold uppercase tracking-widest text-[#F0D4C4]">
                        Feedback de la empresa
                      </p>
                    </div>
                  ...
                  <div className="flex items-center gap-2 text-xs text-gray-400 bg-gray-50 border border-gray-100 rounded-xl px-4 py-3">
                    <i className="fi fi-rr-clock text-[11px]" />
                    Esperando feedback de la empresa...
                  </div>
                )}
              </div>
            )}

            {/* Formulario de entrega — solo etapa actual */}
            {isCurrent && (
              <div className="mt-2">
                ...
                <textarea
                  ...
                  className={`w-full px-4 py-3 border-[1.5px] rounded-xl bg-gray-50 text-sm text-gray-900 outline-none resize-y min-h-28 leading-relaxed transition-all
                    focus:border-[#F26419] focus:bg-white focus:shadow-[0_0_0_3px_rgba(242,100,25,0.1)]
            ...
            {isLocked && (
              <div className="flex items-center gap-2 text-xs text-gray-400 mt-2">
                <i className="fi fi-rr-lock text-[11px]" />
...
export default function Timeline() {
  ...
  const fetchSteps = async () => {
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data.error || "Error al cargar el timeline.");
      }
      const data = await res.json();

      // Calcular qué etapas están desbloqueadas
      // Una etapa se desbloquea si la anterior tiene submission
      const enriched = data.steps.map((step, i) => {
        const prevDone = i === 0 || !!data.steps[i - 1].submission_id;
        return { ...step, unlocked: prevDone };
      });

      setSteps(enriched);

      // Guardar info del job desde la primera etapa si está disponible
  ...
  const handleSubmit = async (stepId, answerText) => {
    ...
    try {
      ...
      // Recargar las etapas para reflejar el nuevo estado
      await fetchSteps();
    } catch (err) {
          ...
          <div className="max-w-3xl mx-auto">
            ...
            <div className="mb-6">
              ...
              <h1 className="text-2xl font-bold text-gray-900 tracking-tight">
                {jobInfo?.title || "Mi proyecto"}
              </h1>
              <p className="text-sm text-gray-400 mt-1">
                Completa cada etapa en orden para avanzar en el proyecto
              </p>
            </div>
            ...
            {allDone && (
              <div className="bg-[#1C1712] rounded-2xl p-6 mb-6 border border-[#F26419]/20 text-center">
                ...
                <p className="text-white/50 text-sm mb-4">
                  Enviaste todas las etapas. Cuando la empresa finalice su feedback podrás descargar tu certificado verificado.
                </p>
                <button
                  disabled
                  className="px-6 py-2.5 bg-[#F26419]/30 text-[#F0D4C4] text-sm font-semibold rounded-xl border-none cursor-not-allowed opacity-60"
                >
                  <i className="fi fi-rr-diploma text-sm mr-2" />
                  Certificado pendiente de feedback
                </button>
              </div>
            )}
