...
function ApplicantCard({ applicant, index, onApprove, onReject, onFeedbackFinal, onViewProfile, updating, token, onViewTimeline}) {
    ...
    <div className="bg-white rounded-2xl border border-gray-100 hover:border-gray-200 transition-all">
      ...
      <div className="flex items-center gap-4 flex-wrap p-5">
        ...
        <div className="flex-1 min-w-0">
          <button onClick={() => onViewProfile(applicant.candidate_id)} className="font-semibold text-gray-900 text-sm truncate cursor-pointer bg-transparent border-none p-0 hover:text-[#E26000] transition-colors text-left block">
        ...
        {applicant.status === "approved" && totalSteps > 0 && (
          <div className="text-center px-3">
            <p className="text-lg font-black text-[#E26000]">
              {steps}/{totalSteps}
            </p>
        ...
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => onViewProfile(applicant.candidate_id)} className="flex items-center gap-1.5 px-3 py-2 bg-gray-50 text-gray-600 border border-gray-200 text-xs font-semibold rounded-xl cursor-pointer transition-all hover:border-[#E26000] hover:text-[#E26000]">
            <i className="fi fi-rr-user text-[11px]" /> Perfil
          </button>


          <button onClick={() => onViewTimeline(applicant.id)} className="flex items-center gap-1.5 px-4 py-2 bg-indigo-500 text-white text-xs font-semibold rounded-xl border-none cursor-pointer transition-all hover:bg-indigo-600 hover:-translate-y-0.5">
            <i className="fi fi-rr-time-forward text-[11px]" />
            Ver timeline
          </button>
      ...
      {applicant.status === "approved" && totalSteps > 0 && (
        <div className="mx-5 mb-4 pt-3 border-t border-gray-50">
          ...
          <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
            <div className="h-full rounded-full transition-all duration-500" style={{ width: `${Math.round((steps / totalSteps) * 100)}%`, background: "linear-gradient(90deg,#E26000,#FF8C3A)" }} />
          </div>
          {allDone && (
      ...
      {showTimeline && (
        <div className="border-t border-gray-100 bg-[#F7F7F8] px-5 py-5 rounded-b-2xl">
          ...
          {!loadingTimeline && timelineSteps.length === 0 && <p className="text-xs text-gray-400 text-center py-4">El candidato aún no tiene entregas.</p>}
          ...
          {!loadingTimeline && timelineSteps.length > 0 && (
            <>
              ...
              <div className="h-1.5 bg-gray-200 rounded-full overflow-hidden mb-5">
                <div className="h-full rounded-full transition-all" style={{ width: `${pct}%`, background: "linear-gradient(90deg,#E26000,#FF8C3A)" }} />
              </div>
                  ...
                  return (
                    <div key={step.step_id} className="flex gap-3">
                      ...
                      <div className="flex flex-col items-center">
                        ...
                        {i < timelineSteps.length - 1 && <div className={`w-px flex-1 mt-1 ${isDone ? "bg-green-200" : "bg-gray-200"}`} style={{ minHeight: 16 }} />}
                      </div>

                      {/* Contenido */}
                      <div
                        className={`flex-1 rounded-xl border overflow-hidden mb-1
                      ...
                      >
                        ...
                        <div className="px-3 py-2.5">
                          <div className="flex items-center justify-between gap-2 mb-1">
                            <p className={`text-xs font-semibold ${isDone ? "text-green-700" : isCurrent ? "text-gray-900" : "text-gray-400"}`}>{step.step_title}</p>
                            <span
                          ...
                          {isDone && step.answer_text && (
                            <div className="bg-gray-50 border border-gray-100 rounded-lg p-2.5 mt-1.5">
                              ...
                              <p className="text-xs text-gray-600 leading-relaxed line-clamp-3 whitespace-pre-line">{step.answer_text}</p>
                              {step.submitted_at && <p className="text-[9px] text-gray-400 mt-1">{new Date(step.submitted_at).toLocaleDateString("es-CO", { day: "numeric", month: "short", year: "numeric" })}</p>}
                            </div>
                          )}
                ...
                <div className="space-y-3">
                  {filtered.map((applicant, i) => (
                    <ApplicantCard
                      ...
                      onViewTimeline={(applicationId) => navigate(`/timeline/${applicationId}`)}
                    />
                  ))}
                </div>
              )}
