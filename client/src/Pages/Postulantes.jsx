...
function ApplicantCard({ applicant, index, onApprove, onReject, onViewProgress, onFeedbackFinal, updating }) {
  ...
  // Todas las etapas completadas = listo para feedback final
    ...
    <div className="bg-white rounded-2xl border border-gray-100 hover:border-gray-200 hover:shadow-sm transition-all p-5">
      <div className="flex items-center gap-4 flex-wrap">
        ...
        <div className="flex-1 min-w-0">
          <p className="font-semibold text-gray-900 text-sm truncate">{applicant.candidate_name}</p>
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {/* Feedback final — solo cuando completó todas las etapas */}
          {allDone && (
      ...
      {/* Mini barra de progreso si está aprobado */}
      {applicant.status === "approved" && totalSteps > 0 && (
...
/* ── Modal de entregas por etapa (solo lectura para empresa) ── */
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-6 overflow-y-auto" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-2xl w-full max-w-2xl my-6 overflow-hidden shadow-2xl">
        ...
        <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
          {loading && <div className="space-y-3">{[1,2].map((i) => (<div key={i} className="animate-pulse bg-gray-50 rounded-xl p-4"><div className="h-4 bg-gray-100 rounded w-1/3 mb-2" /><div className="h-3 bg-gray-100 rounded w-full mb-1" /><div className="h-3 bg-gray-100 rounded w-4/5" /></div>))}</div>}
            ...
            <div key={step.step_id} className="border border-gray-100 rounded-2xl overflow-hidden">
              <div className="bg-gray-50 px-4 py-3 flex items-center justify-between border-b border-gray-100">
                ...
                <span className="text-xs text-gray-400">
                  {new Date(step.submitted_at).toLocaleDateString("es-CO", { day: "numeric", month: "short" })}
                </span>
              </div>
              <div className="p-4">
...
export default function Postulantes() {
  ...
  const [modal, setModal]           = useState(null);        // { type: "progress" | "feedback", applicationId, candidateName }
