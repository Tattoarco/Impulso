    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-6 overflow-y-auto" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-2xl w-full max-w-2xl my-6 overflow-hidden shadow-2xl">
        <div className="bg-[#1C1712] px-6 py-5 flex items-start justify-between">
          ...
          <button onClick={onClose} className="text-white/40 hover:text-white transition-colors bg-none border-none cursor-pointer mt-1">
            ...
            submittedSteps.map((step) => (
              <div key={step.step_id} className="border border-gray-100 rounded-2xl overflow-hidden">
                ...
                <div className="p-4 space-y-4">
                  <div>
                    <p className="text-[11px] font-bold uppercase tracking-widest text-gray-400 mb-2">Entrega del candidato</p>
                    <div className="bg-gray-50 border border-gray-100 rounded-xl p-3">
                      <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-line">{step.answer_text}</p>
                    </div>
                  </div>
                  <div>
                    <p className="text-[11px] font-bold uppercase tracking-widest text-gray-400 mb-2">Tu feedback</p>
                    <textarea
                      value={feedbacks[step.submission_id] || ""}
                      onChange={(e) => setFeedbacks((prev) => ({ ...prev, [step.submission_id]: e.target.value }))}
                      placeholder="Escribe un feedback constructivo y honesto..."
                    ...
                    <div className="flex items-center gap-2 mt-2 mb-3">
                      ...
                      <div className="flex gap-1">
                        {[1, 2, 3, 4, 5].map((n) => (
                          <button key={n} onClick={() => setScores((prev) => ({ ...prev, [step.submission_id]: n }))} className={`text-lg transition-all cursor-pointer bg-none border-none p-0 leading-none hover:scale-110 ${(scores[step.submission_id] || 0) >= n ? "text-amber-400" : "text-gray-200"}`}>
                            <i className={`fi fi-${(scores[step.submission_id] || 0) >= n ? "sr" : "rr"}-star`} />
                          </button>
                        ))}
                      </div>
                    </div>
                    <button
                      ...
                      className={`flex items-center gap-2 px-4 py-2 text-xs font-semibold rounded-xl border-none cursor-pointer transition-all ${saved[step.submission_id] ? "bg-green-500 text-white" : "bg-[#F26419] text-white hover:bg-[#C94E0D] hover:-translate-y-0.5"} disabled:opacity-40 disabled:cursor-not-allowed disabled:transform-none`}
                    >
        ...
        <div className="border-t border-gray-100 px-6 py-4 flex justify-end">
          <button onClick={onClose} className="px-5 py-2.5 text-sm font-medium text-gray-500 bg-gray-50 border border-gray-200 rounded-xl cursor-pointer hover:bg-gray-100 transition-all">
            Cerrar
          </button>
        </div>
      </div>
