...
function CandidateProfileModal({ candidateId, candidateName, token, onClose, navigate }) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // Intentar cargar si tiene portafolio
        const pRes = await fetch(`${API}/api/portfolio/user/${candidateId}`, {
          headers: { Authorization: `Bearer ${token}` },
        }).catch(() => null);
        if (pRes?.ok) {
          const pData = await pRes.json();
          setPortfolio(pData.portfolio || null);
        }
      } catch (err) {
        console.error(err);
      } finally {
  ...
  const habilidades = (() => {
    try {
      return Array.isArray(profile?.habilidades) ? profile.habilidades : JSON.parse(profile?.habilidades || "[]");
    } catch {
      return [];
    }
  })();
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-6" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden">
        {/* Header */}
        <div className="bg-gradient-to-br from-[#F26419] to-rose-500 px-6 py-6">
          <div className="flex items-center justify-between mb-1">
            <p className="text-xs font-bold uppercase tracking-widest text-white/70">Perfil del candidato</p>
            <button onClick={onClose} className="text-white/60 hover:text-white bg-none border-none cursor-pointer text-lg leading-none">
              ✕
            </button>
          </div>
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-2xl bg-white/20 flex items-center justify-center text-white text-2xl font-black border-2 border-white/30">{candidateName?.[0]?.toUpperCase() || "?"}</div>
            <div>
              <h2 className="text-white font-bold text-xl">{candidateName}</h2>
              {profile && (
                <p className="text-white/70 text-sm">
                  {profile.carrera || "Profesional"}
        ...
        {/* Body */}
        <div className="p-6 space-y-4 max-h-[55vh] overflow-y-auto [scrollbar-width:thin]">
          {loading && (
          ...
          {!loading && !profile && <p className="text-sm text-gray-400 italic text-center py-4">Este candidato aún no ha completado su perfil.</p>}

          {!loading && profile && (
            <>
              {profile.bio && (
                <div>
                  <p className="text-[11px] font-bold uppercase tracking-widest text-gray-400 mb-1.5">Sobre mí</p>
              ...
              {(profile.carrera || profile.universidad) && (
                <div>
                  <p className="text-[11px] font-bold uppercase tracking-widest text-gray-400 mb-1.5">Formación</p>
                  <p className="text-sm text-gray-700 font-medium">{profile.carrera}</p>
                  <p className="text-xs text-gray-400">
                    {profile.universidad}
                    {profile.año_graduacion ? ` · ${profile.año_graduacion}` : ""}
                  </p>
                </div>
              )}
              ...
              {habilidades.length > 0 && (
                <div>
                  <p className="text-[11px] font-bold uppercase tracking-widest text-gray-400 mb-2">Habilidades</p>
                  <div className="flex flex-wrap gap-1.5">
                    {habilidades.map((h, i) => (
                      <span key={i} className="text-xs font-medium bg-orange-50 text-[#F26419] border border-orange-100 px-2.5 py-1 rounded-full">
                        {h}
                      </span>
                    ))}
                  </div>
              ...
              {(profile.linkedin || profile.portafolio) && (
                <div>
                  <p className="text-[11px] font-bold uppercase tracking-widest text-gray-400 mb-2">Links</p>
                  <div className="flex flex-col gap-1.5">
                    {profile.linkedin && (
                      <a href={profile.linkedin} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 text-xs text-blue-600 hover:underline font-medium">
                        <i className="fi fi-brands-linkedin text-[11px]" /> LinkedIn
                      </a>
                    )}
                    {profile.portafolio && (
                      <a href={profile.portafolio} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 text-xs text-[#F26419] hover:underline font-medium">
                        <i className="fi fi-rr-globe text-[11px]" /> Portafolio externo
                      </a>
                    )}
                  </div>
                </div>
              ...
              {portfolio && (
                <div className="bg-orange-50 border border-orange-100 rounded-xl p-3 flex items-center justify-between">
                  <div>
                    <p className="text-xs font-semibold text-[#F26419]">Portafolio en Impulso</p>
                    <p className="text-[10px] text-gray-500 mt-0.5">{portfolio.role || "Disponible"}</p>
                  </div>
                  <button
                    onClick={() => {
                      onClose();
                      navigate(`/portafolio/${portfolio.id}`);
                    }}
                    className="text-xs font-semibold px-3 py-1.5 bg-[#F26419] text-white rounded-xl cursor-pointer border-none hover:bg-[#C94E0D] transition-all"
                  >
                    Ver portafolio
                  </button>
                </div>
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 hover:border-gray-200 hover:shadow-sm transition-all p-5">
      <div className="flex items-center gap-4 flex-wrap">
        ...
        <button onClick={() => onViewProfile(applicant.candidate_id, applicant.candidate_name)} className="cursor-pointer bg-none border-none p-0" title="Ver perfil">
...
export default function Postulantes() {
  ...
  return (
    <>
                ...
                <div className="space-y-3">
                  {filtered.map((applicant, i) => (
                    <ApplicantCard
                      ...
                      onViewProfile={(candidateId, name) => setModal({ type: "profile", candidateId, candidateName: name })}
                    />
                  ))}
                </div>
              )}
      ...
      {modal?.type === "profile" && <CandidateProfileModal candidateId={modal.candidateId} candidateName={modal.candidateName} token={token} onClose={() => setModal(null)} navigate={navigate} />}
