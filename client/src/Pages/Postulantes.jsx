...
// ── Reemplaza SOLO la función ApplicantCard en tu Postulantes.jsx ──────

function ApplicantCard({ applicant, index, onApprove, onReject, onFeedbackFinal, onViewProfile, updating, token }) {
  ...
  const toggleTimeline = async () => {
    ...
    if (timelineSteps.length > 0) return; // ya cargado
    setLoadingTimeline(true);
    try {
      const res  = await fetch(`${API}/api/submissions/${applicant.id}`, {
  ...
  const completed = timelineSteps.filter((s) => !!s.submission_id).length;
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 hover:border-gray-200 transition-all">
      ...
      <div className="flex items-center gap-4 flex-wrap p-5">
        ...
        <button onClick={() => onViewProfile(applicant.candidate_id)}
          className="cursor-pointer bg-transparent border-none p-0" title="Ver perfil">
          <Avatar name={applicant.candidate_name} color={color} />
        </button>

        <div className="flex-1 min-w-0">
          <button onClick={() => onViewProfile(applicant.candidate_id)}
            className="font-semibold text-gray-900 text-sm truncate cursor-pointer bg-transparent border-none p-0 hover:text-[#E26000] transition-colors text-left block">
            {applicant.candidate_name}
          </button>
          <p className="text-xs text-gray-400 truncate">{applicant.candidate_email}</p>
          <p className="text-xs text-gray-400 mt-0.5">
            Se postuló el {new Date(applicant.created_at).toLocaleDateString("es-CO", { day: "numeric", month: "short", year: "numeric" })}
          </p>
        </div>

        {applicant.status === "approved" && totalSteps > 0 && (
          <div className="text-center px-3">
            <p className="text-lg font-black text-[#E26000]">{steps}/{totalSteps}</p>
        ...
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => onViewProfile(applicant.candidate_id)}
            className="flex items-center gap-1.5 px-3 py-2 bg-gray-50 text-gray-600 border border-gray-200 text-xs font-semibold rounded-xl cursor-pointer transition-all hover:border-[#E26000] hover:text-[#E26000]">
            <i className="fi fi-rr-user text-[11px]" /> Perfil
          </button>

          {applicant.status === "pending" && (
            <>
              <button onClick={() => onApprove(applicant.id)} disabled={updating === applicant.id}
                className="flex items-center gap-1.5 px-4 py-2 bg-green-500 text-white text-xs font-semibold rounded-xl border-none cursor-pointer transition-all hover:bg-green-600 hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed">
                <i className="fi fi-rr-check text-[11px]" />
                {updating === applicant.id ? "..." : "Aprobar"}
              </button>
              <button onClick={() => onReject(applicant.id)} disabled={updating === applicant.id}
                ...
                <i className="fi fi-rr-cross text-[11px]" /> Rechazar
              </button>
            </>
          )}
          ...
          {applicant.status === "approved" && (
            <button onClick={toggleTimeline}
              className={`flex items-center gap-1.5 px-4 py-2 text-xs font-semibold rounded-xl border-none cursor-pointer transition-all hover:-translate-y-0.5
                ${showTimeline ? "bg-[#1C1712] text-white" : "text-white"}`}
              style={!showTimeline ? { background: "linear-gradient(135deg,#E26000,#FF8C3A)" } : {}}>
          ...
          {allDone && (
            <button onClick={() => onFeedbackFinal(applicant.id, applicant.candidate_name)}
              className="flex items-center gap-1.5 px-4 py-2 bg-[#6651DD] text-white text-xs font-semibold rounded-xl border-none cursor-pointer transition-all hover:bg-[#5542c4] hover:-translate-y-0.5">
              <i className="fi fi-rr-star text-[11px]" /> Feedback final
            </button>
          )}
          ...
          {applicant.status === "rejected" && (
            <span className="flex items-center gap-1.5 px-4 py-2 bg-red-50 text-red-400 text-xs font-semibold rounded-xl border border-red-100">
              <i className="fi fi-rr-cross-circle text-[11px]" /> Rechazado
            </span>
          )}
        </div>
      </div>
      ...
      {/* ── Barra de progreso ── */}
      {applicant.status === "approved" && totalSteps > 0 && (
        <div className="mx-5 mb-4 pt-3 border-t border-gray-50">
          ...
          <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
            <div className="h-full rounded-full transition-all duration-500"
              style={{ width: `${Math.round((steps / totalSteps) * 100)}%`, background: "linear-gradient(90deg,#E26000,#FF8C3A)" }} />
          </div>
          {allDone && (
            <p className="text-[10px] text-[#6651DD] font-semibold mt-1.5 flex items-center gap-1">
      ...
      {/* ── Timeline inline ── */}
      {showTimeline && (
        <div className="border-t border-gray-100 bg-[#F7F7F8] px-5 py-5 rounded-b-2xl">

          {loadingTimeline && (
            <div className="flex items-center gap-2.5 py-4 justify-center">
              <div className="w-5 h-5 border-2 border-gray-200 border-t-[#E26000] rounded-full animate-spin" />
              <p className="text-xs text-gray-400">Cargando timeline...</p>
            </div>
          )}

          {!loadingTimeline && timelineSteps.length === 0 && (
            <p className="text-xs text-gray-400 text-center py-4">El candidato aún no tiene entregas.</p>
          )}

          {!loadingTimeline && timelineSteps.length > 0 && (
            <>
              {/* Mini barra de progreso del timeline */}
              <div className="flex items-center justify-between text-[10px] text-gray-400 mb-1.5">
                <span>{completed} de {total} etapas</span>
                <span className="font-bold text-[#E26000]">{pct}%</span>
              </div>
              ...
              <div className="space-y-3">
                {timelineSteps.map((step, i) => {
                  ...
                  const isLocked  = !isDone && (i > 0 && !timelineSteps[i - 1]?.submission_id);
                  ...
                  return (
                    <div key={step.step_id} className="flex gap-3">
                      ...
                      <div className="flex flex-col items-center">
                        <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[11px] font-bold shrink-0 shadow-sm
                          ${isDone    ? "bg-green-500 text-white" : ""}
                          ${isCurrent ? "text-white" : ""}
                          ${isLocked  ? "bg-gray-200 text-gray-400" : ""}`}
                          style={isCurrent ? { background: "linear-gradient(135deg,#E26000,#FF8C3A)" } : {}}>
                          {isDone ? <i className="fi fi-rr-check text-[9px]" /> : i + 1}
                        </div>
                        {i < timelineSteps.length - 1 && (
                          <div className={`w-px flex-1 mt-1 ${isDone ? "bg-green-200" : "bg-gray-200"}`} style={{ minHeight: 16 }} />
                        )}
                      </div>

                      {/* Contenido */}
                      <div className={`flex-1 rounded-xl border overflow-hidden mb-1
                        ${isDone    ? "bg-white border-green-100" : ""}
                        ${isCurrent ? "bg-white border-[#E26000]/20" : ""}
                        ${isLocked  ? "bg-gray-50 border-gray-100 opacity-60" : ""}`}>

                        {isDone    && <div className="h-1 bg-green-400 rounded-t" />}
                        {isCurrent && <div className="h-1 rounded-t" style={{ background: "linear-gradient(90deg,#E26000,#FF8C3A)" }} />}

                        <div className="px-3 py-2.5">
                          <div className="flex items-center justify-between gap-2 mb-1">
                            <p className={`text-xs font-semibold ${isDone ? "text-green-700" : isCurrent ? "text-gray-900" : "text-gray-400"}`}>
                              {step.step_title}
                            </p>
                            <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full shrink-0
                              ${isDone    ? "bg-green-100 text-green-600" : ""}
                              ${isCurrent ? "bg-[#FEF0E8] text-[#E26000]" : ""}
                              ${isLocked  ? "bg-gray-100 text-gray-400" : ""}`}>
                              {isDone ? "✓ Completado" : isCurrent ? "En progreso" : "Bloqueado"}
                            </span>
                          </div>

                          {isDone && step.answer_text && (
                            <div className="bg-gray-50 border border-gray-100 rounded-lg p-2.5 mt-1.5">
                              <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-1">Entrega</p>
                              <p className="text-xs text-gray-600 leading-relaxed line-clamp-3 whitespace-pre-line">
                                {step.answer_text}
                              </p>
                              {step.submitted_at && (
                                <p className="text-[9px] text-gray-400 mt-1">
                                  {new Date(step.submitted_at).toLocaleDateString("es-CO", { day: "numeric", month: "short", year: "numeric" })}
                                </p>
                              )}
                            </div>
                          )}
                          ...
                          {step.feedback_text && (
                            <div className="bg-[#1C1712] rounded-lg p-2.5 mt-1.5">
                              <p className="text-[10px] font-bold uppercase tracking-widest text-[#E26000] mb-1">Tu feedback</p>
                              <p className="text-xs text-white/70 leading-relaxed">{step.feedback_text}</p>
                            </div>
                          )}
                        </div>
                      </div>
  ...
  export default function Postulantes() {
    ...
    // Cuando se abre el modal de perfil, fetchea el candidato y lo guarda
    ...
    const openProfile = async (candidateId) => {
      ...
      try {
        const res = await fetch(`${API}/api/auth/candidate/${candidateId}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        const data = await res.json();
    ...
    return (
      <>
                  ...
                  <div className="space-y-3">
                    {filtered.map((applicant, i) => (
                      <ApplicantCard
                        ...
                        onViewProfile={(candidateId) => openProfile(candidateId)}
                      />
                    ))}
                  </div>
                )}
        ...
        {/* ProfileModal — usa el componente existente */}
        {modal?.type === "profile" &&
          (loadingProfile ? (
