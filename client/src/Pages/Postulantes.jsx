  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 hover:border-gray-200 hover:shadow-sm transition-all p-5">
      <div className="flex items-center gap-4 flex-wrap">
        ...
        {/* Avatar clickeable */}
        <button onClick={() => onViewProfile(applicant.candidate_id, applicant.candidate_name)}
          className="cursor-pointer bg-none border-none p-0" title="Ver perfil">
        ...
        <div className="flex-1 min-w-0">
          {/* Nombre clickeable */}
          <button onClick={() => onViewProfile(applicant.candidate_id, applicant.candidate_name)}
            className="font-semibold text-gray-900 text-sm truncate cursor-pointer bg-none border-none p-0 hover:text-[#F26419] transition-colors text-left block">
        ...
        {applicant.status === "approved" && totalSteps > 0 && (
          <div className="text-center px-3">
            <p className="text-lg font-black text-[#F26419]">{steps}/{totalSteps}</p>
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {applicant.status === "pending" && (
            <>
              <button onClick={() => onApprove(applicant.id)} disabled={updating === applicant.id}
                ...
                <i className="fi fi-rr-check text-[11px]" />{updating === applicant.id ? "..." : "Aprobar"}
              </button>
          ...
          {applicant.status === "approved" && (
            <button onClick={() => onViewProgress(applicant.id, applicant.candidate_name)}
              ...
              <i className="fi fi-rr-eye text-[11px]" />Ver entregas
            </button>
          )}
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-6 overflow-y-auto" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-2xl w-full max-w-2xl my-6 overflow-hidden shadow-2xl">
        <div className="bg-[#1C1712] px-6 py-5 flex items-start justify-between">
          ...
          <button onClick={onClose} className="text-white/40 hover:text-white bg-none border-none cursor-pointer mt-1 text-lg">✕</button>
        </div>
        <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
          {loading && <div className="space-y-3">{[1,2].map((i) => (<div key={i} className="animate-pulse bg-gray-50 rounded-xl p-4"><div className="h-4 bg-gray-100 rounded w-1/3 mb-2" /><div className="h-3 bg-gray-100 rounded w-full" /></div>))}</div>}
          ...
          {!loading && submittedSteps.map((step) => (
        ...
        <div className="border-t border-gray-100 px-6 py-4 flex justify-end">
          <button onClick={onClose} className="px-5 py-2.5 text-sm font-medium text-gray-500 bg-gray-50 border border-gray-200 rounded-xl cursor-pointer hover:bg-gray-100">Cerrar</button>
        </div>
      </div>
    </div>
  );
...
export default function Postulantes() {
  ...
  const openProfile = async (candidateId) => {
    ...
    } catch { setSelectedCandidate(null); } finally { setLoadingProfile(false); }
  };
  ...
  const updateStatus = async (applicationId, status) => {
    ...
    } catch (err) { showToast("error", err.message); } finally { setUpdating(null); }
  };
  ...
  const filterTabs = [{ key:"all", label:"Todos" },{ key:"pending", label:"Pendientes" },{ key:"approved", label:"Aprobados" },{ key:"rejected", label:"Rechazados" }];
    ...
    <>
            ...
            <div className="flex items-start justify-between gap-4 mb-6 flex-wrap">
              <div>
                ...
                <div className="flex items-center gap-3 mt-2 flex-wrap">
                  {job?.duration     && <span className="flex items-center gap-1 text-xs text-gray-400"><i className="fi fi-rr-clock text-[10px]" /> {job.duration}</span>}
                  {job?.profile_area && <span className="flex items-center gap-1 text-xs text-gray-400"><i className="fi fi-rr-tag text-[10px]" /> {job.profile_area}</span>}
                  <span className="flex items-center gap-1 text-xs text-gray-400"><i className="fi fi-rr-users text-[10px]" /> {applicants.length} postulante{applicants.length !== 1 ? "s" : ""}</span>
                </div>
              </div>
            </div>
                ...
                <div className="flex items-center gap-1 bg-gray-50 border border-gray-100 rounded-xl p-1">
                  {filterTabs.map((f) => (
                    <button key={f.key} onClick={() => setFilter(f.key)}
                      className={`px-3 py-1.5 text-xs font-semibold rounded-lg border-none cursor-pointer transition-all flex items-center gap-1.5 ${filter === f.key ? "bg-white text-[#F26419] shadow-sm" : "bg-transparent text-gray-400 hover:text-gray-700"}`}>
                      {f.label}
              ...
              {!loading && filtered.length === 0 && (
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <p className="text-gray-500 font-medium text-sm mb-1">
                    {filter === "all" ? "Aún no hay postulantes" : `Sin candidatos ${filter === "pending" ? "pendientes" : filter === "approved" ? "aprobados" : "rechazados"}`}
                  </p>
                </div>
              )}
      ...
      {modal?.type === "profile" && (
        loadingProfile ? (
          ...
          <ProfileModal candidate={selectedCandidate} onClose={() => { setModal(null); setSelectedCandidate(null); }} />
        )
      )}

      {modal?.type === "progress" && (
        <ProgressModal applicationId={modal.applicationId} candidateName={modal.candidateName} jobTitle={job?.title || ""} token={token} onClose={() => setModal(null)} />
      )}

      {modal?.type === "feedback" && (
        <FeedbackFinal applicationId={modal.applicationId} candidateName={modal.candidateName} jobTitle={job?.title || ""} token={token} onClose={() => setModal(null)}
          onSaved={() => showToast("success", "Feedback final enviado ✓ El nivel del candidato ha sido actualizado.")} />
      )}
