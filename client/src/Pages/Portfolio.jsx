...
function AIModal({ palette, onClose, onApply }) {
  const [messages, setMessages] = useState([{ role: "ai", text: "¡Hola! 👋 Soy tu asistente de portafolio. Voy a ayudarte a crear una presentación profesional.\n\n¿Cuál es tu nombre completo y en qué área te especializas?" }]);
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col" style={{ height: "600px" }}>
        <div className="px-6 py-4 flex items-center gap-3" style={{ background: palette.accent }}>
          ...
          <button onClick={onClose} className="ml-auto text-white/60 hover:text-white bg-none border-none cursor-pointer text-lg">
            ✕
          </button>
        </div>
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3 [scrollbar-width:thin]">
          ...
          {loading && (
            <div className="flex gap-2.5">
              <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs text-white" style={{ background: palette.accent }}>
                🤖
              </div>
        ...
        {draft && (
          <div className="mx-5 mb-3 p-3 rounded-2xl border-2 text-sm" style={{ borderColor: palette.accent, background: palette.bg }}>
            ...
            <button
              ...
              className="w-full py-2 rounded-xl text-white text-xs font-semibold cursor-pointer border-none hover:opacity-90"
        ...
        <div className="px-5 pb-4 flex gap-2">
          <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()} placeholder="Escribe aquí..." className="flex-1 px-3.5 py-2.5 border-[1.5px] border-gray-200 rounded-full bg-gray-50 text-sm outline-none focus:border-gray-400 transition-all" />
    ...
    <div className="rounded-2xl border bg-white overflow-hidden hover:shadow-md transition-all">
      ...
      <div className="p-4">
        ...
        <p className="text-xs text-gray-400 mb-2">
          {job.company_name} · {job.profile_area}
        </p>
        {job.status === "approved" && job.total_steps > 0 && (
          <div>
            <div className="flex justify-between text-[10px] text-gray-400 mb-1">
              <span>Progreso</span>
              <span>
                {job.steps_completed}/{job.total_steps} etapas
              </span>
            </div>
  ...
  return (
    <div className="rounded-2xl overflow-hidden border bg-white hover:shadow-lg transition-all">
      {project.img ? (
        ...
        <div className="h-36 flex items-center justify-center text-4xl" style={{ background: palette.bg }}>
          🚀
        </div>
      )}
      <div className="p-4">
        ...
        {editMode && (
          <div className="flex gap-2 mt-3">
            <button onClick={onEdit} className="flex-1 text-xs py-1.5 rounded-lg border border-gray-200 text-gray-600 cursor-pointer hover:border-gray-400">
              Editar
            </button>
            <button onClick={onDelete} className="flex-1 text-xs py-1.5 rounded-lg border border-red-200 text-red-500 cursor-pointer hover:bg-red-50">
              Eliminar
            </button>
          </div>
        )}
...
export default function Portfolio() {
  ...
  const viewerIsEmpresa = user?.role === "empresa";
  ...
  useEffect(() => {
    const load = async () => {
      try {
        if (isOwner) {
          const [pRes, jRes, uRes] = await Promise.all([fetch(`${API}/api/portfolio/me`, { headers: { Authorization: `Bearer ${token}` } }), fetch(`${API}/api/portfolio/me/jobs`, { headers: { Authorization: `Bearer ${token}` } }), fetch(`${API}/api/auth/me`, { headers: { Authorization: `Bearer ${token}` } })]);
          ...
          const [pRes] = await Promise.all([fetch(`${API}/api/portfolio/${id}`), fetch(`${API}/api/portfolio/${id}/jobs`).catch(() => ({ json: () => ({ applications: [] }) }))]);
          ...
          if (ownerRes?.ok) {
            ...
            setCandidateData(ownerData.candidate);
          }
        }
      } catch (err) {
        console.error(err);
            ...
            <div className="relative z-10 p-8 flex items-end justify-between gap-6 flex-wrap">
              <div className="flex items-center gap-5">
                ...
                <div>
                  {editMode ? (
                    <>
                      <input value={portfolio.name} onChange={(e) => set("name", e.target.value)} placeholder="Tu nombre" className="bg-white/10 text-white font-bold text-xl border border-white/20 rounded-xl px-3 py-1 outline-none mb-1 block w-full placeholder-white/40" />
                      <input value={portfolio.role} onChange={(e) => set("role", e.target.value)} placeholder="Tu rol profesional" className="bg-white/10 text-white/80 text-sm border border-white/20 rounded-xl px-3 py-1 outline-none block w-full placeholder-white/40" />
                    </>
                  ) : (
                    <>
              ...
              {isOwner && (
                <div className="flex gap-2 flex-wrap">
                  {!editMode && (
                    <button onClick={() => setShowAI(true)} className="flex items-center gap-1.5 px-4 py-2 bg-white/10 text-white text-sm font-medium rounded-xl border border-white/20 cursor-pointer hover:bg-white/20">
                      ✨ Asistente IA
                    </button>
                  )}
                  <button onClick={() => (editMode ? save() : setEditMode(true))} disabled={saving} className="flex items-center gap-1.5 px-4 py-2 bg-white text-sm font-semibold rounded-xl cursor-pointer hover:bg-white/90 disabled:opacity-60" style={{ color: pal.accent }}>
                    {saving ? (
                      <>
                  ...
                  {editMode && (
                    <button onClick={() => setEditMode(false)} className="px-4 py-2 bg-white/10 text-white text-sm font-medium rounded-xl border border-white/20 cursor-pointer hover:bg-white/20">
                      Cancelar
                    </button>
                  )}
                </div>
              ...
              {isOwner && portfolio.id && !editMode && (
                <div className="w-full flex items-center gap-2 bg-white/10 border border-white/20 rounded-xl px-3 py-2 mt-1">
                  ...
                  <button
                    onClick={() => {
                      navigator.clipboard.writeText(`${window.location.origin}/portafolio/${portfolio.id}`);
          ...
          {editMode && (
            <div className="bg-white rounded-2xl border border-gray-100 p-5 mb-6">
              ...
              <div className="flex gap-3 flex-wrap">
                {PALETTES.map((p) => (
                  <button key={p.id} onClick={() => set("bg_color", p.id)} className={`flex items-center gap-2 px-3 py-2 rounded-xl border-2 cursor-pointer text-sm font-medium ${portfolio.bg_color === p.id ? "border-gray-900" : "border-gray-200 hover:border-gray-400"}`}>
          ...
          <div className="grid grid-cols-[1fr_280px] gap-6 max-lg:grid-cols-1">
            ...
            <div className="space-y-6">
              ...
              <section className={`rounded-2xl p-6 ${isDark ? "bg-white/5 border border-white/10" : "bg-white border border-gray-100"} shadow-sm`}>
                ...
                {editMode ? (
                  <textarea value={portfolio.description} onChange={(e) => set("description", e.target.value)} rows={4} placeholder="Cuéntale al mundo quién eres..." className="w-full px-3.5 py-3 border-[1.5px] border-gray-200 rounded-xl bg-gray-50 text-sm outline-none focus:border-gray-400 resize-none leading-relaxed" />
                ) : (
                  <p className={`text-sm leading-relaxed ${isDark ? "text-white/70" : "text-gray-600"}`}>{portfolio.description || <span className="italic text-gray-400">{isOwner ? "¡Edita tu portafolio o usa el asistente IA!" : "Sin descripción aún."}</span>}</p>
                )}
              </section>
              ...
              {jobs.length > 0 && (
                <section className={`rounded-2xl p-6 ${isDark ? "bg-white/5 border border-white/10" : "bg-white border border-gray-100"} shadow-sm`}>
                  <h2 className={`text-sm font-bold uppercase tracking-widest mb-4 flex items-center gap-2 ${isDark ? "text-white/40" : "text-gray-400"}`}>
                    <span className="w-5 h-5 rounded-full flex items-center justify-center text-white text-[10px] font-black" style={{ background: pal.accent }}>
                      I
                    </span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {jobs.map((j, i) => (
                      <ImpulsoJobCard key={i} job={j} palette={pal} />
                    ))}
                  </div>
                </section>
              )}
              ...
              <section className={`rounded-2xl p-6 ${isDark ? "bg-white/5 border border-white/10" : "bg-white border border-gray-100"} shadow-sm`}>
                <div className="flex items-center justify-between mb-4">
                  ...
                  {editMode && (
                    <button onClick={() => setEditingProject({ title: "", description: "", url: "", img: "" })} className="flex items-center gap-1 text-xs font-semibold px-3 py-1.5 rounded-xl text-white cursor-pointer border-none hover:opacity-90" style={{ background: pal.accent }}>
                      <i className="fi fi-rr-plus text-[10px]" /> Agregar
                    </button>
                  )}
                </div>
                {(portfolio.customprojects || []).length === 0 && !editMode && <p className={`text-sm italic ${isDark ? "text-white/30" : "text-gray-400"}`}>{isOwner ? "Agrega proyectos personales para mostrar tu trabajo." : "Sin proyectos agregados aún."}</p>}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {(portfolio.customprojects || []).map((proj, i) => (
                    <CustomProjectCard key={i} project={proj} palette={pal} editMode={editMode} onEdit={() => setEditingProject({ ...proj, _idx: i })} onDelete={() => deleteProject(i)} />
                  ))}
                  {editMode && (
                    <button onClick={() => setEditingProject({ title: "", description: "", url: "", img: "" })} className="h-48 border-2 border-dashed border-gray-200 rounded-2xl flex flex-col items-center justify-center gap-2 text-gray-400 hover:border-gray-400 hover:text-gray-600 cursor-pointer bg-transparent">
            ...
            <div className="space-y-5">
              {/* NIVEL IMPULSO — nuevo */}
              {!viewerIsEmpresa && <NivelCard nivel={nivel} puntos={puntos} />}
              {/* Skills */}
              <section className={`rounded-2xl p-5 ${isDark ? "bg-white/5 border border-white/10" : "bg-white border border-gray-100"} shadow-sm`}>
                      ...
                      {editMode && (
                        <button
                          onClick={() =>
                            set(
                              "skills",
                              portfolio.skills.filter((_, j) => j !== i),
                            )
                          }
                          className="ml-0.5 opacity-70 hover:opacity-100 cursor-pointer bg-none border-none text-white leading-none"
                        >
                          ×
                        </button>
                      )}
                ...
                {editMode && (
                  <div className="flex gap-1.5">
                    <input value={skillInput} onChange={(e) => setSkillInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && (e.preventDefault(), addSkill(skillInput))} placeholder="Ej: Figma" className="flex-1 px-3 py-1.5 border border-gray-200 rounded-xl bg-gray-50 text-xs outline-none focus:border-gray-400" />
                    <button onClick={() => addSkill(skillInput)} className="px-3 py-1.5 rounded-xl text-white text-xs font-medium cursor-pointer border-none hover:opacity-90" style={{ background: pal.accent }}>
                      +
                    </button>
                  </div>
                )}
              ...
              <section className={`rounded-2xl p-5 ${isDark ? "bg-white/5 border border-white/10" : "bg-white border border-gray-100"} shadow-sm`}>
                ...
                <div className="space-y-2">
                      ...
                      {editMode ? (
                        <>
                          <input
                            value={l.label}
                            onChange={(e) => {
                              const list = [...portfolio.links];
                              list[i] = { ...l, label: e.target.value };
                              set("links", list);
                            }}
                            placeholder="Nombre"
                            className="w-20 px-2 py-1.5 border border-gray-200 rounded-lg bg-gray-50 text-xs outline-none"
                          />
                          <input
                            value={l.url}
                            onChange={(e) => {
                            ...
                            placeholder="https://..."
                            className="flex-1 px-2 py-1.5 border border-gray-200 rounded-lg bg-gray-50 text-xs outline-none"
                          />
                          <button
                            onClick={() =>
                              set(
                                "links",
                                portfolio.links.filter((_, j) => j !== i),
                              )
                            }
                        ...
                        <a href={l.url} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 text-xs font-medium hover:underline" style={{ color: pal.accent }}>
                          <i className="fi fi-rr-link text-[10px]" />
                          {l.label || l.url}
                        </a>
                      )}
                    </div>
                  ...
                  {editMode && (
                    <button onClick={() => set("links", [...(portfolio.links || []), { label: "", url: "" }])} className="text-xs mt-1 cursor-pointer bg-none border-none font-medium" style={{ color: pal.accent }}>
                      + Agregar link
                    </button>
                  )}
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      ...
                      <div key={label} className={`rounded-xl p-3 text-center ${isDark ? "bg-white/5" : "bg-gray-50"}`}>
                        <p className="text-2xl font-black" style={{ color: pal.accent }}>
                          {value}
                        </p>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl">
            ...
            <div className="flex gap-3 mt-5">
              <button onClick={() => setEditingProject(null)} className="flex-1 py-2.5 border border-gray-200 rounded-xl text-sm font-medium text-gray-600 cursor-pointer hover:bg-gray-50">
                Cancelar
              </button>
              <button onClick={() => saveProject({ title: editingProject.title, description: editingProject.description, url: editingProject.url, img: editingProject.img }, editingProject._idx !== undefined ? editingProject._idx : "new")} className="flex-1 py-2.5 rounded-xl text-white text-sm font-semibold cursor-pointer border-none hover:opacity-90" style={{ background: pal.accent }}>
                Guardar
              </button>
            </div>
          </div>
