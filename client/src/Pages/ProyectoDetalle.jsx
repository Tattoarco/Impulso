    ...
    <div className="animate-pulse space-y-6">
      <div className="bg-white rounded-2xl p-8 border border-gray-100">
        ...
        <div className="flex gap-3 mb-6">
          {[20, 24, 16].map((w, i) => (
            <div key={i} className={`h-5 bg-gray-100 rounded-full w-${w}`} />
          ))}
        </div>
        {[100, 80, 60].map((w, i) => (
          <div key={i} className={`h-3 bg-gray-100 rounded w-${w === 100 ? "full" : w === 80 ? "4/5" : "3/5"} mb-2`} />
        ))}
      </div>
    </div>
  );
      ...
      <div className="flex min-h-screen bg-[#F7F7F8]">
        <main className="ml-24 flex-1 p-8 flex items-center justify-center">
          <div className="text-center">
            ...
            <button onClick={() => navigate(-1)} className="text-sm text-[#E26000] underline cursor-pointer bg-transparent border-none">
              Volver atrás
            </button>
          </div>
        </main>
    ...
    <>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm sticky top-8">
                  ...
                  {!appStatus && isCandidate && (
                    <button onClick={handleApply} disabled={applying} className="w-full py-3.5 text-white font-bold text-sm rounded-xl border-none cursor-pointer transition-all hover:-translate-y-0.5 hover:shadow-[0_6px_20px_rgba(226,96,0,0.3)] disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none mb-3" style={{ background: "linear-gradient(135deg, #E26000, #FF8C3A)" }}>
                      {applying ? (
      ...
      disabled:opacity-50 disabled:cursor-not-allowed"
                  ...
                  {!isCandidate && !localStorage.getItem("token") && (
                    <button onClick={() => navigate("/login")} className="w-full py-3.5 text-white font-bold text-sm rounded-xl border-none cursor-pointer mb-3" style={{ background: "linear-gradient(135deg, #E26000, #FF8C3A)" }}>
