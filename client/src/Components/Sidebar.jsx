...
export default function Sidebar() {
  ...
  const ctaRoute     = user?.role === "empresa" ? "/empresa/crear-proyecto" : "/dashboard";
  const ctaLabel     = user?.role === "empresa" ? "Crear proyecto" : "Explorar proyectos";
      ...
      <div className="h-full bg-white/80 backdrop-blur-xl rounded-3xl shadow-xl flex flex-col justify-between p-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-6">
            {open && user && (
              <div className="flex items-center gap-3">
                <img
                  src={`https://ui-avatars.com/api/?name=${encodeURIComponent(user.name)}&background=F26419&color=fff`}
          ...
          <div className="space-y-1">
            <Item icon="fi fi-rr-home"      text="Dashboard"       open={open} onClick={() => navigate("/dashboard")} />
            <Item icon="fi fi-rr-briefcase" text="Mis proyectos"   open={open} onClick={() => navigate(misProyectosRoute)} />
            {user?.role === "candidato" && (
              <Item icon="fi fi-rr-user" text="Mi perfil" open={open} onClick={() => navigate("/candidato/perfil")} />
            )}
        ...
        <div className="space-y-2">
          ...
          {open ? (
            <Button onClick={() => navigate(ctaRoute)} className="w-full bg-orange-500 text-white rounded-2xl py-3 flex items-center justify-center gap-2 shadow-md">
              <span className="text-lg">+</span> {ctaLabel}
            </Button>
          ) : (
            <Button onClick={() => navigate(ctaRoute)} className="w-full bg-orange-500 text-white rounded-xl py-3 flex justify-center">+</Button>
          )}
        </div>
      </div>
    </div>
...
function Item({ icon, text, open, onClick }) {
  return (
    <div onClick={onClick} className="flex items-center gap-3 px-3 py-2 rounded-xl cursor-pointer transition text-gray-600 hover:bg-gray-100">
